...
os.environ['WANDB_API_KEY']='0f9e58e8e0dbc136de3d75e7ed388013f4001691'
...
def train(
    ...
    device='cuda:1',
    model_save_name='diff-basic-butterflies',
    wandb_project='diff-basic',
    log_samples_every = 50,
    save_model_every = 100,
    ):
        
    # Initialize wandb for logging
    wandb.init(project=wandb_project, config=locals()) # locals 返回输入的所有参数
    ...
    sampling_scheduler = DDIMScheduler.from_pretrained(start_model)
    ...
    for epoch in range(num_epochs):
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            ...
            timesteps = torch.randint(0, image_pipe.scheduler.config.num_train_timesteps, (bs,), device=clean_images.device).long()
            # import pdb
            # pdb.set_trace()
        ...
        # Update the learning rate for the next epoch # 别把这个和noise scheduler 搞混了
        scheduler.step()
